...
test('permanent program retains all four prescriptions, load policies and 74 working sets',()=>{
  assert.deepEqual(MAIN_WORKOUTS.map(w=>w.exercises),BLOCK_WORKOUTS.map(w=>w.exercises));
  assert.equal(MAIN_WORKOUTS.flatMap(w=>w.exercises).reduce((n,e)=>n+e.sets,0),74);
