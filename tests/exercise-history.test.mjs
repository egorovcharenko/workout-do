...
test("pull-ups prefill from global per-exercise history, never a same-workout snapshot", () => {
  const lastSession = {
  ...
  const globalHints = {
    "Pull-Ups|working|0": row(4),
    "Pull-Ups|working|1": row(4),
    "Pull-Ups|working|2": row(3),
    "Pull-Ups|working|3": row(2),
  };
  ...
  assert.deepEqual(Object.keys(history).map(Number), [0, 1, 2, 3]);
  assert.equal(history[0].reps, "4");
  assert.equal(history[3].reps, "2");
});
