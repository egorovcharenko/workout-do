...
test("lat pulldowns, low rows, and calf raises use two cable stacks", () => {
  ...
  assert.equal(cableStackMultiplier("Calf Raises"), 2);
  assert.equal(cableStackMultiplier("Calf Raise"), 2);
});
...
test("low rows are recognized as cable-stack exercises even without equipment metadata", () => {
  ...
  assert.equal(isCableStackExercise("Calf Raises"), true);
...
test("effective load doubles current per-stack values only for dual-stack movements", () => {
  ...
  assert.equal(effectiveExerciseWeight("Calf Raises", 50), 100);
