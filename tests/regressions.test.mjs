...
test("sparkline scale focuses on training values and the goal, not a deload outlier", () => {
  const points = [
    { value: 179 },
    { value: 181 },
    { value: 130, isDeload: true },
  ];
  const domain = sparklineDomain(points, 180);

  assert.ok(domain.min > 175);
  assert.ok(domain.max < 185);
