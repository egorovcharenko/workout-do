...
export const BLOCK_PROGRESSION_NOTES = [
  ...
  "Progress throughout the block. Reach the upper range with reserve on two consecutive appearances, then accept the smallest practical load increase. Each set or matching group progresses separately; a missed target does not automatically lower your weights.",
...
export function buildTrainingBlockWorkouts(regularWorkouts, revision = TRAINING_BLOCK_PRESCRIPTION_REVISION) {
  ...
  return [
    workout("strength-a", BLOCK_NAMES[0], "A · Squat & Bench", [
      exercise("Barbell Back Squat", [135, 115, 115], [[5, 8], [8, 10], [8, 10]], 180, 5,
