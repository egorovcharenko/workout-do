...
export function trainingBlockSessionMatches(session, block) {
  return block?.permanent ? isTrainingBlockSession(session)
    : !!block && parseSessionState(session?.state_json)?.trainingBlock?.instanceId === block.instanceId;
}
...
export function resolveTrainingBlockSession(workout, block, savedSession, regularWorkouts) {
  ...
  return {
    workout: { ...(buildTrainingBlockWorkouts(regularWorkouts, revision).find(item => item.id === workout.id) || workout),
      ...(workout.permanent ? { permanent: true, program: true, hidden: false } : {}) },
    block: { ...(savedSession ? saved || block : { ...block, prescriptionRevision: revision }),
      ...(workout.permanent ? { permanent: true } : {}) },
  };
}

// A newly assigned exercise can bootstrap from its existing slot in this run.
// Once logged in the destination workout, that workout owns its progression.
export function trainingBlockSeedSession(workout, exercise, sessions, block) {
  if (!exercise?.historySourceWorkout) return null;
  return (sessions || []).filter(session => session.workout_name === exercise.historySourceWorkout
    && trainingBlockSessionMatches(session, block) && trainingBlockPrescriptionMatches(session, workout)
    && isStoredSessionFinished(session) && !session.is_deload
    && (session.sets || []).some(set => set.exercise === exercise.name && set.set_type === "working"))
    .sort((a, b) => String(b.started_at || b.date).localeCompare(String(a.started_at || a.date)))[0] || null;
}

// Loads come from the last time each exercise was done in any program
// workout (Pull-Ups in A or B), not only from the last session of this workout.
export function trainingBlockHints(workout, sessions, block, excludeSessionId = null) {
  const completed = (sessions || []).filter(session => session.id !== excludeSessionId
    && trainingBlockSessionMatches(session, block)
  ...
  if (revised) {
    ...
    for (const exercise of workout.exercises) {
      const latest = trainingBlockSeedSession(workout, exercise, sessions, block)
        || baseline.find(session => (session.sets || []).some(set => set.exercise === exercise.name && set.set_type === "working"));
      if (!latest) continue;
...
export function buildTrainingBlockWorkouts(regularWorkouts, revision = TRAINING_BLOCK_PRESCRIPTION_REVISION) {
  ...
  const core = () => exercise("Dragon Fly Progression", [null, null, null], [3, 8], 75, null,
  ...
  const pullUps = () => exercise("Pull-Ups", [0, 0, 0, 0], null, 150, null,
  ...
  const workout = (id, name, label, exercises, main) => ({ id, name, blockLabel: label, exercises,
    ...
    main, program: false, hidden: true, kind: main ? "main" : "micro", rest: main ? 150 : 90 });
  return [
    workout("strength-a", BLOCK_NAMES[0], "A · Squat & Bench", [
      exercise("Barbell Back Squat", [135, 135, 115], [[5, 8], [6, 10], [8, 10]], 150, 5,
        { warmups: 3, defaultWarmup: [45, 95, 115], defaultWarmupReps: [8, 3, 1],
      ...
      exercise("Barbell Bench Press", [150, 145, 145, 125], [[3, 5], [5, 10], [5, 10], [8, 12]], 150, 2,
        { warmups: 3, defaultWarmup: [45, 95, 125], defaultWarmupReps: [8, 5, 2] }),
      pullUps(),
      exercise("Standing Overhead Press", [95, 95, 95], [4, 10], 150, 2,
        { warmups: 1, defaultWarmup: [45], defaultWarmupReps: [8] }),
      exercise("Lat Pulldown", [60, 55, 55], [6, 12], 90, 1.25),
    ], true),
    workout("strength-accessories-1", BLOCK_NAMES[1], "Accessories 1 · Dips & Arms", [
      exercise("Dips", [25, 25, 25], [5, 12], 150, 2.5),
      exercise("Bayesian Cable Curl", [20, 20, 20], [6, 15], 75, 1.25),
      exercise("Single-Arm Cable Rear Delt Fly", [15, 15, 15], [8, 15], 75, 1.25,
        { historySourceWorkout: "Strength Accessories 2" }),
      exercise("Dumbbell Hammer Curls", [30, 30, 30], [5, 12], 75, 2.5),
      exercise("Overhead Tricep Extension", [35, 35, 35], [6, 15], 75, 1.25), core(),
      { ...WRIST_CURL },
    ], false),
    workout("strength-b", BLOCK_NAMES[2], "B · RDL & Incline", [
      exercise("Barbell RDL", [205, 205, 165], [[4, 8], [4, 8], [8, 15]], 150, 5,
        { targetRir: [2, 2], warmups: 3, defaultWarmup: [95, 135, 165], defaultWarmupReps: [5, 3, 1] }),
      exercise("Incline Barbell Press", [135, 135, 115, 115], [[4, 8], [4, 8], [6, 12], [6, 12]], 150, 2,
        { warmups: 2, defaultWarmup: [45, 95], defaultWarmupReps: [8, 5] }),
      pullUps(),
      exercise("Neutral-Grip Lat Pulldown", [60, 60, 60], [6, 12], 90, 1.25),
      exercise("Low Row", [65, 65, 60], [6, 12], 150, 1.25),
    ], true),
    workout("strength-accessories-2", BLOCK_NAMES[3], "Accessories 2 · Delts & Traps", [
      { ...CABLE_CHEST_FLY },
      exercise("Single-Arm Cable Lateral Raise", [15, 15, 10], [6, 15], 75, 1.25),
      exercise("Barbell Shrugs", [135, 205, 205, 205], [[10, 12], [6, 15], [6, 15], [6, 15]], 90, 5),
      exercise("Single-Arm Cable Rear Delt Fly", [15, 15, 15], [8, 15], 75, 1.25),
      exercise("Incline DB Curls", [20, 20, 20], [6, 15], 75, 2.5),
      exercise("Cable Tricep Pushdowns", [55, 55, 55], [5, 12], 75, 1.25),
      exercise("Calf Raises", [55, 55, 55], [15, 20], 75, 1.25), core(),
      { ...WRIST_CURL },
    ], false),
  ];
}
