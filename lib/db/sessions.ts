...
import { beltAdjustedRepScore, isAssistExercise, isRepsOnlyExercise } from "@/lib/legacy/standards";
...
export async function get1RMHistory(uid: string): Promise<{
  ...
  for (const s of sessions) {
    for (const set of s.sets ?? []) {
      ...
      // Reps-only exercise: graded on reps (mirrors calcSet1RM). Explicitly
      // typed belt loads fold into the score as equivalent bodyweight reps so
      // adding belt weight registers as progress even at the same rep count.
      if (isRepsOnlyExercise(set.exercise)) {
        const beltWeight = isStoredBeltLoad(set) ? storedBeltLoad(set) : 0;
        push(ormRaw, key, beltWeight > 0 ? beltAdjustedRepScore(reps, beltWeight) : reps);
        if (beltWeight > 0) {
