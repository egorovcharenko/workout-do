...
import { effectiveExerciseWeight, effectiveStoredExerciseWeight } from "./cable-stack.js";

const EXERCISE_MUSCLES = {
  "Bench-Supported Dumbbell Wrist Curls": { primary: ["forearms"], secondary: [], ratios: { forearms: 1.0 } },
  ...
  "Bayesian Cable Curl": { primary: ["biceps"], secondary: ["forearms"], ratios: { biceps: 1.0, forearms: 0.3 } },
  ...
  "Cable Chest Fly": { primary: ["chest"], secondary: [], ratios: { chest: 1.0 } },
  ...
  "Lat Pulldown": { primary: ["lats"], secondary: ["biceps", "upper_back", "rear_delts"], ratios: { lats: 1.0, biceps: 0.35, upper_back: 0.4, rear_delts: 0.3 } },
  "Neutral-Grip Lat Pulldown": { primary: ["lats"], secondary: ["biceps", "upper_back", "rear_delts"], ratios: { lats: 1.0, biceps: 0.4, upper_back: 0.4, rear_delts: 0.3 } },
  ...
  "Single-Arm Cable Rear Delt Fly": { primary: ["rear_delts"], secondary: ["upper_back"], ratios: { rear_delts: 1.0, upper_back: 0.2 } },
...
const DRAGONFLY_STAGE_RANK = { "tuck": 1, "single-leg": 2, "tuck-extension": 3, "straddle": 4, "negatives": 5, "dragon-flag": 6, "dragon-fly": 7 };
...
// Belted reps-only sets (Dips / Pull-Ups with plates on a belt) would score as
// raw reps and hide progress whenever reps hold steady while the belt grows.
// Convert (bodyweight + belt) × reps into equivalent bodyweight reps via Epley
// so the score stays continuous with historic rep-only points: belt 0 → exact
// reps; any belt > 0 → a little more than the raw reps.
// Only explicitly typed belt rows use this (load_type === "belt") — older rows
// that stored bodyweight in weight_lb keep scoring as raw reps.
const REPS_ONLY_BODYWEIGHT_LB = 175;
function beltAdjustedRepScore(reps, beltLb, bodyweightLb = REPS_ONLY_BODYWEIGHT_LB) {
  const r = Math.max(0, parseInt(reps) || 0);
  const belt = Math.max(0, Number(beltLb) || 0);
  if (!belt || !r) return r;
  const bw = bodyweightLb > 0 ? bodyweightLb : REPS_ONLY_BODYWEIGHT_LB;
  const score = ((bw + belt) * (1 + r / 30) / bw - 1) * 30;
  return Math.round(score * 10) / 10;
}

function calcSet1RMFromEffectiveWeight(exerciseName, effectiveWeight, reps, bandsJson, grip, loadType) {
  if (isRepsOnlyExercise(exerciseName)) {
    if (loadType === "belt") return beltAdjustedRepScore(reps, effectiveWeight);
    return reps;
  }
...
function calcSet1RM(exerciseName, weight, reps, bandsJson, grip, loadType) {
  return calcSet1RMFromEffectiveWeight(
    exerciseName,
    effectiveExerciseWeight(exerciseName, weight),
    reps,
    bandsJson,
    grip,
    loadType,
...
function calcStoredSet1RM(exerciseName, weight, reps, bandsJson, grip, session, loadType) {
  return calcSet1RMFromEffectiveWeight(
    exerciseName,
    effectiveStoredExerciseWeight(exerciseName, weight, session),
    reps,
    bandsJson,
    grip,
    loadType,
...
if (typeof window !== "undefined") {
  ...
  window.beltAdjustedRepScore = beltAdjustedRepScore;
...
export {
  ...
  calcSet1RM, calcStoredSet1RM, applySwaps, isRepsOnlyExercise, isAssistExercise,
  beltAdjustedRepScore,
};
