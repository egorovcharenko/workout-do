...
export function followupWeightPatch(set, weight) {
  // Re-selecting the current load (e.g. just activating the set from the nav)
  // is not a manual choice and must keep the set following its source.
  if (weight === set.weight) return { weight };
