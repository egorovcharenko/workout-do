...
function cableStackMultiplier(exerciseName) {
  if (exerciseName === "Cable Chest Fly") return 2;
...
// Smallest stack weight the pin + add-ons can show that is at least `minimum`
// (capped at the heaviest). Progression offers must land on one of these, or
// the stack selector snaps them back down to the previous load.
function nextCableStackWeightAtLeast(minimum) {
  const target = Number(minimum) || 0;
  let best = CABLE_STACK_MAX_LOAD;
  for (let pin = CABLE_STACK_MIN; pin <= CABLE_STACK_MAX; pin += CABLE_STACK_STEP) {
    for (let addOns = 0; addOns <= CABLE_STACK_ADD_ON_COUNT; addOns += 1) {
      const candidate = roundCableWeight(pin + addOns * CABLE_STACK_ADD_ON_WEIGHT);
      if (candidate >= target - 0.001 && candidate < best) best = candidate;
    }
  }
  return best;
}
