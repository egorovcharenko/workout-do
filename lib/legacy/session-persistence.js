...
import { isPendingSet, nextSupersetTarget } from "./set-logging";
...
function setSessionStateCache(workoutName, date, stateObj) {
  ...
  _sessionStateCache[`${workoutName}:${date}`] = {
    ...
    trainingBlock: stateObj.trainingBlock || null,
  };
}

function setSessionTrainingBlock(workoutName, date, block) {
  const key = `${workoutName}:${date}`;
  if (!_sessionStateCache[key]) _sessionStateCache[key] = {};
  _sessionStateCache[key].trainingBlock = block || null;
}
...
function clearSessionState(workoutName, date) {
  delete _sessionStateCache[`${workoutName}:${date}`];
}
...
function serializeForSave(exercises, workoutName, sessionId, startedAt, elapsed, activeDate) {
  ...
  const stateObj = {
    ...
    ...(cached.trainingBlock ? { trainingBlock: cached.trainingBlock } : {}),
    setsMap: setsMap
  };
...
// Save status for the header indicator: "idle" | "saving" | "saved" | "error".
let _saveStatus = "idle";
const _saveStatusListeners = new Set();
function _setSaveStatus(status) {
  if (_saveStatus === status) return;
  _saveStatus = status;
  _saveStatusListeners.forEach(listener => listener(status));
}
function getSaveStatus() {
  return _saveStatus;
}
function onSaveStatus(listener) {
...
// It also mints a replacement doc when a sent id no longer exists or its
// update is rejected as a stale-tab / cross-workout write. Remember the id
// the server handed back and stamp it onto any later payload for the same
// workout+date — including payloads queued while they still carried the older
// id — so every follow-up save reuses the live doc (queued autosaves, the
// finish save racing the first autosave).
let _resolvedSessionKey = null;
let _resolvedSessionId = null;
function _withSessionId(payload) {
  if (_resolvedSessionId &&
      `${payload.workout}:${payload.date}` === _resolvedSessionKey &&
      payload.session_id !== _resolvedSessionId) {
...
// Set while an abandon deletes the draft for this workout+date (and after it
// succeeds). A save still on the wire can land after the delete; saveSession
// then finds no doc and creates a new one, so such late saves delete whatever
// they wrote instead of resurrecting the abandoned workout.
let _abandonedKey = null;
async function autoSavePayload(payload, onResolved) {
  if (TEST_MODE) return;
  if (_abandonedKey === `${payload.workout}:${payload.date}`) return;
  if (_saveInFlight) {
  ...
  try {
    ...
    if (res.ok && _abandonedKey === `${payload.workout}:${payload.date}`) {
      await api.deleteSession(res.id);
    } else if (res.ok) {
    ...
    _saveInFlight = false;
    if (!_savePending) _setSaveStatus(failed ? "error" : "saved");
    if (_savePending) {
...
// Abandoning is deliberately stronger than navigating home: it drops queued
// autosaves, waits for an in-flight create/update to settle, then deletes the
// server draft (including an id resolved while we were waiting). This avoids
// a late autosave recreating the workout after the user abandoned it.
async function abandonSession(workoutName, date, sessionId) {
  const sessionKey = `${workoutName}:${date}`;
  _savePending = null;

  // Give an in-flight save a moment to settle so its id is known. The wait is
  // bounded because a stale/offline client request can hang forever. A loaded
  // session already has a durable id, so it can be deleted through the
  // authenticated server route anyway; a save that lands later is cleaned up
  // by autoSavePayload via _abandonedKey.
  const deadline = Date.now() + 3000;
  while (_saveInFlight && Date.now() < deadline) {
    await new Promise(r => setTimeout(r, 50));
  }
  if (_saveInFlight && !sessionId) {
    throw new Error("Timed out waiting for the workout draft to finish saving");
  }

  const resolvedId = _resolvedSessionKey === sessionKey ? _resolvedSessionId : null;
  // Prefer the resolved id: if the loaded document disappeared and an
  // autosave recreated it, sessionId is stale while resolvedId is current.
  const idToDelete = resolvedId || sessionId;
  _abandonedKey = sessionKey;
  try {
    if (idToDelete) await api.deleteSession(idToDelete);
  } catch (e) {
    _abandonedKey = null;
    throw e;
  }

  clearSessionState(workoutName, date);
  if (_resolvedSessionKey === sessionKey) {
...
function hydrateToday(exercises, todaySets) {
  ...
  if (todaySets && todaySets.length) {
    todaySets.forEach(row => {
      ...
      set.rir = row.rir === undefined ? (set.rir ?? null) : row.rir;
      set.completed = true;
...
function activateNextSet(exercises) {
  ...
  // Superset groups pick their member by round-robin (fewest completed working
  // sets first) so a reload mid-superset resumes on the correct partner
  // instead of always re-selecting the first one.
  const seenGroups = new Set();
  for (const ex of exercises) {
    if (ex.skipped) continue;
    if (ex.superset) {
      if (seenGroups.has(ex.superset)) continue;
      seenGroups.add(ex.superset);
      const target = nextSupersetTarget(exercises, ex.superset);
      if (target) {
        exercises[target.eIdx].sets[target.sIdx].active = true;
        return exercises;
      }
      continue;
    }
...
export {
  setSessionStateCache, setSessionTrainingBlock, loadSwaps, saveSwaps, loadPmStarted, savePmStarted,
  ...
  clearSessionState, autoSavePayload, finishSavePayload, abandonSession, hydrateToday, activateNextSet,
  getSaveStatus, onSaveStatus,
};
