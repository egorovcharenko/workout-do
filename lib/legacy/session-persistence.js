...
async function abandonSession(workoutName, date, sessionId) {
  ...
  // A loaded session already has a durable id, so the authenticated server
  // route can delete it without waiting on this browser's Firestore queue.
  // Waiting here can hang forever on a stale/offline client request.
  if (!sessionId) {
