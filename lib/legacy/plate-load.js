...
function barbellPlateChanges(fromWeight, toWeight, bar = BARBELL_BAR, previousPlates = null) {
  const from = previousPlates ?? decomposeBarbellLoad(fromWeight, bar);
  const to = restackBarbellLoad(from, toWeight, bar);
  let shared = 0;
...
function nearestBarbellWeightByPlateChanges(fromWeight, targetWeight, { tolerance = 5, step = 5, bar = BARBELL_BAR, previousPlates = null } = {}) {
  ...
  for (let candidate = first; candidate <= target + tolerance + 0.0001; candidate += step) {
    ...
    const changes = barbellPlateChanges(fromWeight, weight, bar, previousPlates);
