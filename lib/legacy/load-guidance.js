...
import { cableStackMultiplier, nextCableStackWeightAtLeast } from "./cable-stack.js";
...
export function withLoadGuidance(exercise, config, history, workoutName) {
  ...
  for (const numbers of policy.groups) {
    ...
    const raised = Math.round((fromWeight + policy.increment) * 100) / 100;
    const offer = {
      fromWeight,
      weight: (exercise.equipment ?? config.equipment) === "cable" ? nextCableStackWeightAtLeast(raised) : raised,
