  ...
  for (const numbers of policy.groups) {
    ...
    for (const session of history) {
      ...
      const qualifies = !session.is_deload && !state?.skipped?.includes(exercise.name) && group.every(set => {
        ...
        return !storedSet?.userSkipped && storedSet?.planTargetReps == null
    ...
    const offer = {
      ...
      qualifying, required: 2, ready: qualifying === 2,
      canApply: qualifying === 2 && group.every(pending), practice: !!policy.practice,
