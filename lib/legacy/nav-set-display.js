...
function baseNavSetDisplay(s, exercise) {
  // Logged reps and unlogged targets are distinct. Do not show a template's
  // starting number as though it were a previous result.
  const previewReps = s.repGuidance
    ? s.repGuidance.rangeLabel ?? s.repGuidance.suggested ?? null
    : s.lastReps ?? null;
  const weightMultiplier = cableStackMultiplier(exercise.name);
  if (exercise.repsOnly && !exercise.beltLoad) {
    const targetRange = parseRepTargetRange(exercise.repRange, s.kind === "work" ? s.setNumber - 1 : null);
    ...
    if (s.active) return { repsOnly: true, reps: s.reps ?? previewReps ?? targetReps, state: "current", kind: s.kind };
    return { repsOnly: true, reps: previewReps ?? targetReps, state: "upcoming", preview: true, kind: s.kind };
  }
  if (exercise.stages) {
    ...
    if (s.active) {
      const reps = s.reps ?? previewReps;
      return { lb: cur || prev, reps, state: "current", kind: s.kind };
    }
    return { lb: cur || prev, reps: previewReps, state: "upcoming", preview: true, kind: s.kind };
  }
  ...
  const shown = currentLoadPresent ? cur : prevW;
  return { lb: exercise.beltLoad ? (shown > 0 ? `+${shown}` : "BW") : shown, reps: previewReps, state: "upcoming", preview: true, kind: s.kind, weightMultiplier };
}

function navSetDisplay(s, exercise) {
  const previous = s.repGuidance?.previous;
  const display = {
    ...baseNavSetDisplay(s, exercise),
    lastReps: previous?.reps ?? null,
    lastTitle: previous ? `${previous.date} · ${previous.label}` : null,
  };
  return !s.completed && s.reps == null && s.repGuidance?.rirLabel && !s.repGuidance.rangeLabel
    ? { ...display, reps: null, rirLabel: s.repGuidance.rirLabel }
    : display;
}
