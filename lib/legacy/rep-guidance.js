...
import { regularProgramSessions, trainingBlockSessionMatches, trainingBlockPrescriptionMatches, trainingBlockSeedSession } from "../training-block.js";
...
export function withRepGuidance(exercises, sessions, { workout, block = null, sessionId = null, date, startedAt = null } = {}) {
  ...
  // Any program workout counts: "Last" is the last time this exercise was done.
  const scoped = block ? eligible.filter(session => trainingBlockSessionMatches(session, block) && trainingBlockPrescriptionMatches(session, workout)) : regular;
  return exercises.map(exercise => {
    const appeared = session => (session.sets || []).some(row => row.exercise === exercise.name && row.set_type === "working" && repCount(row.reps));
    // The first appearance of a lift in the program can still show the real
    // previous program result. Later appearances use the latest session with it.
    const scopedSession = scoped.find(appeared);
    const config = workout?.exercises?.find(ex => ex.name === exercise.name);
    const previousSession = scopedSession || (block ? trainingBlockSeedSession(workout, config, eligible, block) || regular.find(appeared) : null);
    const regularRir = !block && workout?.program
      ? exercise.name === "Barbell RDL" || exercise.stages ? [2, 2] : [1, 2] : null;
    const sets = exercise.sets.map(set => {
      const warmup = set.kind === "warmup";
      const targetRir = !warmup && !exercise.deload && set.planTargetReps == null
        ? block ? config?.targetRir : regularRir : null;
      const prescribed = warmup ? config?.defaultWarmupReps?.[set.setNumber]
        : set.planTargetReps ?? config?.defaultWorkReps?.[set.setNumber - 1];
      const rirOnly = targetRir && (block ? !config?.workRepRanges?.[set.setNumber - 1] : exercise.name === "Pull-Ups");
      const range = rirOnly ? null : warmup ? (prescribed ? [prescribed, prescribed] : null)
        : set.planTargetReps != null ? [set.planTargetReps, set.planTargetReps]
        : set.targetRepRange || parseRepTargetRange(exercise.repRange, set.setNumber - 1);
    ...
    const progressionHistory = block && workout?.prescriptionRevision >= 2
      ? [...scoped.filter(session => session.workout_name === workout.name || appeared(session)), ...regular.filter(session => session.date < (block.resumeDate || block.startDate) && appeared(session))]
      : scoped.filter(session => session.workout_name === workout.name || appeared(session));
