import { nearestBarbellWeightByPlateChanges, restackBarbellLoad } from "./plate-load.js";
...
// How far (lb) the derived back-off may drift from 90% of the top set to save plate swaps.
const SQUAT_BACKOFF_TOLERANCE = 5;
 ...
 * weights and anchoring the back-offs at roughly 90% of the top set, picking
 * the nearby 5 lb step that needs the fewest plate changes from the top set. Subsequent
...
function applySquatProgression(sets, { hasHistory = true } = {}) {
  ...
  // Before the workout starts, project the same stack the visualizer will
  // build through the warm-ups. The live top-set stack refines this on log.
  let projectedPlates = [];
  for (const set of sets) {
    if (set === top) break;
    if (!set.userSkipped) projectedPlates = restackBarbellLoad(projectedPlates, set.weight);
  }
  projectedPlates = restackBarbellLoad(projectedPlates, topWeight);
  const migratedBackoffWeight = nearestBarbellWeightByPlateChanges(
    topWeight,
    topWeight * SQUAT_BACKOFF_RATIO,
    { tolerance: SQUAT_BACKOFF_TOLERANCE, previousPlates: projectedPlates },
  );
...
function optimizeMigratedSquatBackoffs(exercise, loggedIndex, previousPlates) {
  const top = exercise.sets[loggedIndex];
  if (!previousPlates || exercise.name !== SQUAT_EXERCISE || exercise.deload || exercise.planPrescribed
    || !exercise.progression?.backoff?.migrated || top?.progressionRole !== "top") return exercise;
  const backoffs = exercise.sets.filter(set => set.progressionRole === "backoff");
  // Never rewrite logged sets or a weight the user changed explicitly.
  if (backoffs.length !== 2 || backoffs.some(set => set.completed || set.userSkipped
    || !sameWeight(set.weight, exercise.progression.backoff.weight))) return exercise;
  const weight = nearestBarbellWeightByPlateChanges(top.weight, top.weight * SQUAT_BACKOFF_RATIO,
    { tolerance: SQUAT_BACKOFF_TOLERANCE, previousPlates });
  return { ...exercise,
    sets: exercise.sets.map(set => set.progressionRole === "backoff" ? { ...set, weight } : set),
    progression: { ...exercise.progression,
      backoff: { ...exercise.progression.backoff, weight },
      detail: `Keep the top set at ${top.weight} lb, then use ${weight} lb for two higher-quality back-offs. This replaces the fatiguing straight sets while keeping three work sets.`,
    },
  };
}

export {
  optimizeMigratedSquatBackoffs,
  SQUAT_BACKOFF_RANGE,
  SQUAT_EXERCISE,
  SQUAT_TOP_RANGE,
  SQUAT_WEIGHT_STEP,
