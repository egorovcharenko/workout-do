...
/** All exercises in the supplied completed history, not just the latest workout. */
export function buildAllTime1rmTrends(history = [], { bodyweightLb } = {}) {
  const trends = Object.create(null);
  for (const session of [...history].sort((a, b) => a.date.localeCompare(b.date)
    || String(a.started_at || '').localeCompare(String(b.started_at || '')))) {
    for (const name of new Set((session.sets || []).map(row => row.exercise))) {
      if (!name || !supports1rm(name)) continue;
      const result = best1rm(session, name, bodyweightLb);
      if (!result || !validTrendPoint({ date: session.date, ...result })) continue;
      (trends[name] ||= []).push({ date: session.date, ...result });
    }
  }
  return trends;
}
...
/** Summaries chart only the last `months` calendar months (counted back from
 * the newest point). A series with nothing that recent keeps its last point so
 * the current value still shows. */
export function limitTrendsToRecentMonths(trends = {}, months = 3) {
  const dates = Object.values(trends).flat().filter(validTrendPoint).map(point => point.date).sort();
  if (!dates.length) return trends;
  const cutoff = new Date(`${dates.at(-1).slice(0, 7)}-01T00:00:00Z`);
  cutoff.setUTCMonth(cutoff.getUTCMonth() - (months - 1));
  const from = cutoff.toISOString().slice(0, 10);
  return Object.fromEntries(Object.entries(trends).map(([name, points]) => {
    if (!Array.isArray(points)) return [name, points];
    const recent = points.filter(point => point.date >= from);
    return [name, recent.length ? recent : points.slice(-1)];
  }));
}
...
export function renderRecap1rm(points, timeDomain = null) {
  ...
  const inspection = chartInspectionAttributes(coords.map(point => ({
    date: point.date, x: point.x / 160 * 100,
    values: [
      { label: '1RM est', value: number(point.value), unit: 'lb', kind: 'estimate' },
      ...(validWeight(point.maxWeight) ? [{ label: 'Max weight', value: Number(point.maxWeight.toFixed(2)), unit: 'lb', kind: 'weight' }] : []),
    ],
  })), basis ? 'Includes bodyweight' : '');
  return `<div class="recap-1rm" aria-label="Estimated 1RM and maximum weight history" aria-description="${basis}" ${inspection}>
    ...
    <svg role="img" aria-label="Estimated 1RM across ${points.length} ${points.length === 1 ? 'workout' : 'workouts'}, latest ${number(last.value)} lb${validWeight(last.maxWeight) ? `; maximum working-set weight ${number(last.maxWeight)} lb, shared weight scale` : ''}" >
      ...
      <g class="recap-max-weight">
        ${coords.some((p, i) => i > 0 && validWeight(p.maxWeight) && validWeight(coords[i - 1].maxWeight)) ? `<path d="${weightPath}" fill="none" stroke="currentColor" stroke-width="2" vector-effect="non-scaling-stroke" stroke-linejoin="round" stroke-linecap="round"/>` : ''}
      </g>
      </svg>
