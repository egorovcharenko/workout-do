...
export function renderRecap1rm(points, timeDomain = null) {
  ...
  return `<div title="${basis}" class="recap-1rm" aria-label="Estimated 1RM history">
    ...
    <svg role="img" aria-label="Estimated 1RM across ${points.length} ${points.length === 1 ? 'workout' : 'workouts'}, latest ${number(last.value)} lb${validWeight(last.maxWeight) ? `; maximum working-set weight ${number(last.maxWeight)} lb, shared weight scale` : ''}" viewBox="0 0 160 44" preserveAspectRatio="none">
