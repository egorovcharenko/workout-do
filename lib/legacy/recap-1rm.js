...
export function renderRecap1rm(points) {
  ...
  const monthBars = months.filter(month => month.delta != null && month.delta !== 0 && firstDate !== lastDate).map(month => {
    ...
    const height = Math.abs(month.delta) / maxChange * 18;
    const label = `${month.month}: estimated 1RM ${month.delta > 0 ? '+' : ''}${number(month.delta)} lb (${month.fromDate} to ${month.toDate})`;
    return `<rect class="recap-month-change ${month.delta > 0 ? 'gain' : 'loss'}" x="${(left + gap).toFixed(2)}" y="${(month.delta > 0 ? 22 - height : 22).toFixed(2)}" width="${Math.max(0, right - left - 2 * gap).toFixed(2)}" height="${height.toFixed(2)}" rx="1" role="img" aria-label="${label}"><title>${label}</title></rect>`;
  }).join('');
  ...
  return `<div title="${basis}" class="recap-1rm" aria-label="Estimated 1RM history">
    ...
    <svg role="img" aria-label="Estimated 1RM across ${points.length} ${points.length === 1 ? 'workout' : 'workouts'}, latest ${number(last.value)} lb${validWeight(last.maxWeight) ? `; maximum working-set weight ${number(last.maxWeight)} lb, shared weight scale` : ''}${monthBars ? '; background bars show monthly estimated 1RM change, green increases and red decreases' : ''}" viewBox="0 0 160 44">
      <title>Best estimated 1RM and maximum working-set weight per workout · shared lb scale · all history through ${last.date}. Monthly change bars use a separate scale centered at zero; first month and months after gaps compare their first and last results.</title>
      ${monthBars ? `<g class="recap-month-bars"><line x1="5" x2="155" y1="22" y2="22" class="recap-month-zero"/>${monthBars}</g>` : ''}
    ...
    <div class="recap-1rm-dates"><span>${points[0].date.slice(2, 7)}</span><span>${points.length > 1 ? last.date.slice(2, 7) : 'First result'}</span></div>
  </div>`;
}
