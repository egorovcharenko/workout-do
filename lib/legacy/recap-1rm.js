...
/** A full-width monthly strip keeps the actual changes readable on phones. */
export function renderRecapMonthlyChanges(points = []) {
  const months = recapMonthlyChanges(points);
  ...
  const cells = [];
  for (const date = new Date(months[0].start); date.getTime() < months.at(-1).end; date.setUTCMonth(date.getUTCMonth() + 1)) {
    ...
    const height = delta == null ? 0 : delta === 0 ? 1 : Math.abs(delta) / maxChange * 14;
    ...
    cells.push(`<div class="recap-month-cell ${kind}" title="${title}" aria-label="${title}">
      ...
      <span class="recap-month-name">${monthLabel(date)} <small>${key.slice(2, 4)}</small></span>
    </div>`);
  }
  return `<div class="recap-month-summary"><div class="recap-month-heading">1RM CHANGE · LB</div><div class="recap-month-grid">${cells.join('')}</div></div>`;
}
