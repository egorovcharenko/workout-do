...
import { GRIP_LABELS, interleavedSetNumber } from "./shared";
...
function applyDeloadPrescription(exercises) {
  ...
  return exercises.map(ex => {
    ...
    let single;
    if (ex.assist || ex.isBandsOnly) {
      // Bodyweight (Pull-Ups, Dips, ...) and band-only: 80% isn't selectable —
      // one set exactly like the normal first set (same assistance/bands, reps).
      single = { ...basis, idx: 1 };
    } else {
    ...
    const cappedWarmups = (ex.assist || ex.isBandsOnly) ? warmups
