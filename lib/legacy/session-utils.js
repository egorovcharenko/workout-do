...
function flattenTemplate(workout, lastSessionMap, hintsMap) {
  ...
  workout.exercises.forEach((ex, exIdx) => {
    if (ex.supersetExercises) {
      ...
      const workingCount = Math.max(ex.sets || 3, lastPerformedCount);
      ...
      const fallbackForWorking = (idx) => {
        return previousWorkingSet(ex.name, workingLastBySetNum, idx, idx + 1);
      };
      const workGripFallback = lookupExerciseGrip(ex.name);
      if (ex.name === PULL_UPS_EXERCISE) {
      ...
      } else if (ex.name === SQUAT_EXERCISE) {
