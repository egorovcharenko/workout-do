...
import { addLiveExerciseTime } from "./duration-estimates";
import { BENCH_EXERCISE, applyBenchProgression } from "./bench-progression";
import { isBeltLoadExercise, storedBeltLoad } from "./belt-load";
import {
  PULL_UPS_EXERCISE,
  previousWorkingSet,
...
function computeSessionTimes(exercises, startedAtMs, activeExerciseIdx = null, nowMs = null) {
  const entries = [];
  exercises.forEach((ex, ei) => (ex.sets || []).forEach(s => {
    if (s.completed && s.logged_at) {
      const t = Date.parse(s.logged_at);
      if (!isNaN(t)) entries.push({ ei, key: s.logged_at, t });
    }
  }));
  entries.sort((a, b) => a.t - b.t);
  const bySet = {}, byExercise = {};
  entries.forEach((x, i) => {
    const prev = i === 0
      ? (startedAtMs && startedAtMs < x.t && (x.t - startedAtMs) < 7200000 ? startedAtMs : x.t - 120000)
      : entries[i - 1].t;
    const d = Math.max(0, Math.round((x.t - prev) / 1000));
    bySet[x.key] = d;
    byExercise[x.ei] = (byExercise[x.ei] || 0) + d;
  });
  const lastLoggedMs = entries.length ? entries[entries.length - 1].t : null;
  return {
    bySet,
