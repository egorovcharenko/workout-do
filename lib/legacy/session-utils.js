...
import { currentSessionExerciseTimes } from "./duration-estimates";
import { BENCH_EXERCISE, applyBenchProgression } from "./bench-progression";
import { SQUAT_EXERCISE, applySquatProgression } from "./squat-progression";
import { isBeltLoadExercise, lastBeltLoadFromHistory, storedBeltLoad } from "./belt-load";
import { transitionActiveSetAfterLog } from "./set-logging";
import { clampCableStackWeight } from "./cable-stack";
import {
  PULL_UPS_EXERCISE,
  previousWorkingSet,
...
function flattenTemplate(workout, lastSessionMap, hintsMap) {
  ...
  workout.exercises.forEach((ex, exIdx) => {
    if (ex.supersetExercises) {
      ...
      // A "+ set" on a superset adds a full round for every partner, so the
      // round count also follows the last performed count, floored at template.
      const rounds = Math.max(
        ex.sets || 3,
        ...subs.map(sub => (hintsMap.__counts || {})[sub.name] || 0),
      );
      subs.forEach((sub, subIdx) => {
        ...
        const subBeltLoad = isBeltLoadExercise(sub.name);
        const subIsBandOnly = sub.equipment === "band" && !sub.bandAddon && !subIsAssist && !subIsRepsOnly;
        ...
        out.push({
          ...
          beltLoad: subBeltLoad,
          isBarbell: sub.equipment === "barbell",
      ...
      const beltLoad = isBeltLoadExercise(ex.name);
      const isBandOnly = ex.equipment === "band" && !ex.bandAddon && !isAssist && !isRepsOnly;
      ...
      const workingSource = selectWorkingHistorySource(ex.name, lastSessionMap, hintsMap);
      const allWorkingHistory = workingHistoryBySetNumber(ex.name, workingSource);
      const lastExerciseBeltLoad = beltLoad ? lastBeltLoadFromHistory(ex.name, hintsMap) : null;
      const workingLastBySetNum = { ...allWorkingHistory };
      if (ex.name === PULL_UPS_EXERCISE) delete workingLastBySetNum[0];
      const sortedHistNums = Object.keys(workingLastBySetNum).map(Number).sort((a, b) => a - b);
      // Sets added mid-session persist: prescribe as many working sets as the
      // last session that included this exercise, floored at the template.
      const lastPerformedCount = (hintsMap.__counts || {})[ex.name] || 0;
      const workingCount = ex.fixedPrescription && !ex.preserveSetCount ? ex.sets : Math.max(ex.sets || 3, lastPerformedCount);
      const stageAttempts = ex.stages
        ? sortedHistNums.slice(0, workingCount).map((setNumber) => ({
            stageId: workingLastBySetNum[setNumber]?.grip || null,
            reps: parseInt(workingLastBySetNum[setNumber]?.reps, 10) || null,
          }))
        : null;
      const fallbackForWorking = (idx) => {
        return ex.fixedPrescription ? (workingLastBySetNum[idx + 1] || null)
          : previousWorkingSet(ex.name, workingLastBySetNum, idx, idx + 1);
      };
      const workGripFallback = lookupExerciseGrip(ex.name);
      if (ex.name === PULL_UPS_EXERCISE && !ex.fixedPrescription) {
        const uaSet = buildSet({
          kind: "work", idx: "UA",
          template: ex,
          last: allWorkingHistory[0] || null,
          setNumber: 0,
          saveExerciseName: ex.name,
          isAssist: true,
          isBandOnly: false,
        ...
        uaSet.bands = [];
        uaSet.lastBands = [];
        if (lastExerciseBeltLoad != null && !ex.fixedPrescription) {
          uaSet.weight = lastExerciseBeltLoad;
          uaSet.lastWeight = lastExerciseBeltLoad || undefined;
        }
        sets.push(uaSet);
      }
      for (let i = 0; i < workingCount; i++) {
        ...
        if (lastExerciseBeltLoad != null && !ex.fixedPrescription) {
          workSet.weight = lastExerciseBeltLoad;
          workSet.lastWeight = lastExerciseBeltLoad || undefined;
        }
        sets.push(workSet);
      }
      let progression = null;
      if (ex.name === BENCH_EXERCISE && !ex.fixedPrescription) {
        const bench = applyBenchProgression(sets, { hasHistory: sortedHistNums.length > 0 });
        sets.splice(0, sets.length, ...bench.sets);
        progression = bench.progression;
      } else if (ex.name === SQUAT_EXERCISE && !ex.fixedPrescription) {
        const squat = applySquatProgression(sets, { hasHistory: sortedHistNums.length > 0 });
        sets.splice(0, sets.length, ...squat.sets);
      ...
      out.push({
        ...
        repsOnly: isRepsOnly,
        beltLoad,
        isBarbell: ex.equipment === "barbell",
        equipment: ex.equipment || null,
        session: ex.session || null,
        progression,
        planPrescribed: !!ex.fixedPrescription,
        sets,
      });
    }
  });
...
function buildSet({ kind, idx, template, last, setNumber, saveExerciseName, isAssist, isBandOnly, fallbackGrip }) {
  const set = { kind, idx, setNumber, saveExerciseName, completed: false, active: false, reps: null, rir: null };
  const beltLoad = !!template.beltLoad || isBeltLoadExercise(template.name);
  ...
  if (last) {
    ...
    if (template.repsOnly) {
      // Old reps-only rows stored bodyweight in weight_lb. Only the explicit
      // belt marker is safe to restore as added load.
      if (beltLoad) set.lastWeight = storedBeltLoad(last) || undefined;
    }
  ...
  if (template.repsOnly) {
    if (beltLoad) set.weight = set.lastWeight || 0;
    set.bands = [];
    ...
    const isB = template.equipment === "barbell";
    set.weight = set.lastWeight || defW || (isB ? 45 : 0);
    set.bands = template.bandAddon ? [...set.lastBands] : [];
    if (template.grips) set.grip = set.lastGrip || template.grips[0];
  }
  if (template.fixedPrescription) {
    if (template.prescriptionVersion) set.prescriptionVersion = template.prescriptionVersion;
    if (defW != null && !template.assist && !isBandOnly && (!last || kind === "warmup")) set.weight = defW;
    if (kind === "warmup" && defR != null) set.lastReps = defR;
    if (kind === "work") {
      set.targetRepRange = template.workRepRanges?.[setNumber - 1];
      if (!last && defR != null) set.lastReps = defR;
    }
  }
  if (template.name === PULL_UPS_EXERCISE && template.grips?.length === 1) set.grip = template.grips[0];
  return set;
}

const safeJSON = (s) => { try { return JSON.parse(s); } catch (_) { return []; } };

// Per-set / per-exercise time spent this session, derived from each logged
// set's logged_at (same chain logic as getExDurs on the history page): a set's
// duration is the gap since the previously logged set, or since session start
// for the first one (nominal 2 min when the start is unknown or absurdly far).
// The active exercise also receives the live gap since the last logged set so
// "actual" keeps ticking instead of freezing between set submissions.
// Returns { bySet: { [logged_at]: seconds }, byExercise: { [exIdx]: seconds } }.
function computeSessionTimes(exercises, startedAtMs, activeExerciseIdx = null, nowMs = null) {
  return currentSessionExerciseTimes(exercises, startedAtMs, activeExerciseIdx, nowMs);
}

const fmtSetDuration = (sec) => sec == null ? null : sec >= 90 ? `${Math.round(sec / 60)}m` : `${sec}s`;
...
// the top normal work weight, rounded UP to 5 lb (to the nearest stack load
// for cables) but always lighter than normal when a lighter load exists. The
// kept set retains its setNumber so save/hydrate keys stay consistent across reload.
function deloadWorkWeight(maxW, { isBarbell = false, isCable = false } = {}) {
  if (isCable) return clampCableStackWeight(maxW * 0.8);
  let w = Math.ceil(maxW * 0.8 / 5) * 5;
  // Rounding up erases the 20% cut on light loads (15 → 12 → 15); step down
  // to the next 5 below the normal weight instead.
  if (w >= maxW && maxW > 5) w = Math.floor((maxW - 0.001) / 5) * 5;
  return isBarbell ? Math.max(45, w) : w;
}
...
function applyDeloadPrescription(exercises) {
  return exercises.map(ex => {
    ...
    if (ex.assist || ex.isBandsOnly || ex.repsOnly) {
      ...
      const w = deloadWorkWeight(maxW, { isBarbell: ex.isBarbell, isCable: ex.equipment === "cable" });
...
function applyPlanPrescription(exercises, entry) {
  ...
  const applySetTargets = (set, ps, ex) => {
    const next = { ...set, completed: false, active: false, reps: null, rir: null };
    if (ps.w != null && !ex.assist && !ex.isBandsOnly && (!ex.repsOnly || ex.beltLoad)) {
      next.weight = ps.w;
    ...
    if (ps.r != null) {
      next.planTargetReps = ps.r;
      if (ex.name !== PULL_UPS_EXERCISE) next.lastReps = ps.r;
    }
    return next;
  };
  ...
  items.filter(it => it.add && !exercises.some(ex => byName(ex.name, it.name))).forEach((it, k) => {
    ...
    const exShell = {
      ...
      beltLoad: template.beltLoad,
      isBarbell: template.equipment === "barbell",
...
export {
  ...
  applyPlanPrescription, computeSessionTimes, fmtSetDuration,
};
