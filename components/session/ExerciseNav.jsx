...
function ExerciseNav({ exercises, durationMeta, shownIdx, currentIdx, onSelect, onSelectSet, onSwapExercise, onAddExercise, variant, isFinished }) {
  ...
  const doneSets = exercises.reduce((n, e) => e.skipped ? n + e.sets.length : n + e.sets.filter(isResolvedSet).length, 0);
  const donePct = totalSets ? Math.round((doneSets / totalSets) * 100) : 0;
  const groups = [];
  ...
  return (
    <React.Fragment>
      <div className="nav-rail" style={{ padding: 12, borderRadius: 14, background: T.cardBg, border: `1px solid ${T.cardBorder}` }}>
        <div className="nav-rail-label" style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, marginBottom: 6, padding: "0 2px" }}>
          <span style={{ color: T.faint, fontFamily: T.mono, fontSize: 9, fontWeight: 800, letterSpacing: 1.0 }}>EXERCISES</span>
          <span style={{ fontFamily: T.mono, fontSize: 10, fontWeight: 800, color: totalSets && doneSets === totalSets ? T.green : T.accentLight }}>
            {doneSets}/{totalSets} SETS · {donePct}%
          </span>
        </div>
        <div className="nav-rail-progress" style={{ height: 3, borderRadius: 99, background: "rgba(255,255,255,0.06)", margin: "0 2px 10px", overflow: "hidden" }}>
          <div style={{ height: "100%", width: `${donePct}%`, background: totalSets && doneSets === totalSets ? T.green : T.accent, borderRadius: 99, transition: "width 240ms ease" }} />
        </div>
        <div className="nav-rail-list" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {groups.map((g, gi) => {
            if (!g.superset) {
              const i = g.items[0];
            ...
            return (
              <div key={`g${gi}`} style={{
                ...
                <div className="nav-group-bar" style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px 8px", flexWrap: "wrap" }}>
                  <span style={{
                  ...
                  {g.items.map((i, k) => {
                    ...
                    return (
                      <React.Fragment key={exercises[i].id}>
                        {k > 0 && (
                          <div className="nav-then" style={{ display: "flex", justifyContent: "center", margin: "7px 0" }}>
                            <span style={{
          ...
          <div className="nav-add" style={{ marginTop: 12, borderTop: `1px solid ${T.cardBorder}`, paddingTop: 12 }}>
            <button onClick={() => setShowAddLibrary(true)} style={{
