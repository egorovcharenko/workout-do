...
function BarbellVisualizer({ weight, onWeightChange, compact = false, sets = [], setIndex = 0 }) {
  const PLATE_COLORS = {
  ...
  const loadedPlates = platesForExerciseSet(sets.map((set, index) => index === setIndex ? { ...set, weight } : set), setIndex);
