...
function Header({ workout, workouts, onPickWorkout, onAbandon, done, total, elapsedSec, durationMeta, deload, homeHref = "/" }) {
  ...
  const handleAbandon = async () => {
    ...
    const confirmed = window.confirm(
      "Abandon this workout? All progress from this session will be permanently deleted."
    );
    if (!confirmed) return;
    setAbandoning(true);
    try {
      ...
      window.alert("Could not abandon the workout. Your progress is still saved; please try again.");
      setAbandoning(false);
    }
  };
                ...
                {onAbandon && (
                  <div style={{ borderTop: `1px solid ${T.cardBorder}`, marginTop: 4, paddingTop: 4 }}>
                    <button
                      type="button"
                      role="menuitem"
                      onClick={() => { setOpen(false); handleAbandon(); }}
                      disabled={abandoning}
