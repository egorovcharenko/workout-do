...
function App() { const [workoutId, setWorkoutId] = useState(() => { const fromUrl = new URLSearchParams(window.location.search).get("w");
  ...
  useEffect(() => { let cancelled = false;
    ...
    (async () => { try { const results = await Promise.allSettled([ api.settings(), api.todaySession(workout.name), api.hints(), api.allHistory(), api.history1RM() ]);
        ...
        setSwaps(swapMap); let exs = flattenTemplate(applySwaps(workout, swapMap), {}, hints || {});
        ...
        <div className="session-main">
          ...
          {shownIdx === null && isFinished ? ( <WorkoutCompleteScreen
            ...
            const card = ( <ExerciseCard exercise={ex}
                ...
                onPickWeight={(sIdx, w, plates) => actions.onPickWeight(i, sIdx, w, undefined, plates)}
            ...
            return ( <React.Fragment key={ex.id}>
                <div style={{ margin: "0 16px 12px", background: T.cardBg, border: "1px solid rgba(192,132,252,0.25)", borderRadius: 14, overflow: "hidden", }}>
                  <div className="ss-group-bar" style={{ padding: "10px 14px", display: "flex", alignItems: "center", gap: 8, background: "rgba(192,132,252,0.06)" }}>
                    <span style={{ color: T.bands, fontFamily: T.mono, fontSize: 10, fontWeight: 800, letterSpacing: 1.2 }}>
                      ⇄ SUPERSET {ex.superset}
                    </span>
                  ...
                  {group.map((m, k) => { const tag = `${ex.superset}${k + 1}`;
                    if (m.idx === i) { return ( <div key={m.idx} className="ss-active-member" style={{ padding: "12px 14px 14px", borderTop: k > 0 ? divider : "none" }}>
                    ...
                    const last = fmtLast(m); return ( <div key={m.idx} className="ss-partner-row" onClick={() => onSelectExercise(m.idx)} style={{ padding: "10px 14px", borderTop: k > 0 ? divider : "none", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", opacity: allDone ? 0.55 : 1, }}>
                  ...
                  {nextPartner && (
                    <div className="ss-round-hint" style={{ padding: "9px 14px", borderTop: divider, background: "rgba(255,255,255,0.02)", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
