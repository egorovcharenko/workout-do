...
function renderWorkoutSummaryCard() {
  ...
  (latest.sets || []).forEach(set => {
    ...
    sum.setsCount++;
    const est = calcStoredSet1RM(ex, recordedWeight, r, set.bands_json, set.grip, latest, set.load_type);
    ...
    history.forEach(s => {
      ...
      (s.sets || []).forEach(set => {
        if (set.exercise === exName && set.set_type === 'working' && set.reps) {
          ...
          const w = effectiveStoredExerciseWeight(exName, recordedWeight, s);
          const est = calcStoredSet1RM(exName, recordedWeight, r, set.bands_json, set.grip, s, set.load_type);
...
export { renderWorkoutSummaryCard };
