...
export function renderLatestWorkoutRecap(history, activeSessions = [], today = localDate(), options = {}) {
  ...
  return `<article class="workout-recap home-workout-recap" aria-label="Latest workout recap">
    ...
    <ol class="workout-recap-exercises">${recap.exercises.map(exercise => `<li class="workout-recap-exercise">
      ...
      <div class="workout-recap-results${trends[exercise.name] ? ' has-trend' : ''}"><div>${exercise.groups.length ? exercise.groups.map(group => `<div class="workout-recap-set-group"><span class="workout-recap-load">${escapeHtml(group.load)}</span><span class="workout-recap-reps"><span class="workout-recap-times">× </span><strong>${group.reps.join(' · ')}</strong><small> reps</small></span></div>`).join('') : `<p class="workout-recap-warmup-only">${exercise.warmupSets} warm-up ${exercise.warmupSets === 1 ? 'set' : 'sets'} only</p>`}</div>${renderRecap1rm(trends[exercise.name])}</div>${renderRecapMonthlyChanges(trends[exercise.name])}
    </li>`).join('')}</ol>
